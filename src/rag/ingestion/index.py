...
from src.ingestion.utils import partition_document,analyze_elements,separate_content_types,get_page_number,create_ai_summary
...
def download_content_and_partition(document_id: str, document: dict):
    ...
    try:
        ...
        if document_source_type == "url":
            ...
            response = scrapedo_client.get(url)
...
def vectorize_chunks_summary_and_store_in_database(processed_chunks, document_id):
    ...
    try:
     ...
     for start in range(0,len(ai_summary_list),batch_size):
        ...
        while True:
            try:
                ...
                all_vectorized_embeddings.extend(embeddings) #extend is used to append the data at the end of the list
     ...
     # Step 2 : Storing Chunks with Embeddings in Database
     # chunk_embedding_pairs: list of tuples (chunk, embedding)
     # [
     ...
     # zip function is used to combine the processed_chunks and all_vectorized_embeddings into a list of tuples
